...
Milestone 3A completes the three confirmed readiness fixes and LAB 006 timer mismatch / LAB 007 incorrect installed next hop, following the [2E audit](engine-audit.md). See [execution](milestone-3a.md) and [verification](verification.md). The [Learning Academy audit](learning-academy-audit.md) is planning only: its module pilot needs separate authorization. LAB 008 incorrect mask retains its own addressing/proxy-ARP gate. Immutable revisions must precede mutations to published content or variants. Non-chain topology and new protocols remain later work.
...
No LAB 008, Academy expansion, FlagForge, general switch emulator, authentication system or LLM grading is included. No deployment is authorized as part of 3A.
